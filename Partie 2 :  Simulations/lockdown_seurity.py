...
pygame.init()
pygame.display.set_caption(' lockdown simulation by stopping affected people ')
...
while running:
    ...
    for person in people:
        if person.status == 'healthy' or person.status == 'sick' or person.status == 'immune':
             person.move()
        if person.status == 'sick' or person.status == 'sick_stop':
    ...
    if data[0][len(data[0]) - 1] == 0 and not graphed:
        ...
        colors = ['green', 'red', 'yellow', 'blue', 'blue']
